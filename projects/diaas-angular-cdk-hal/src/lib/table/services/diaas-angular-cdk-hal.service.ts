...
export class HalResourceService {
  ...
  getCollectionHandlers(){
     if (this.resource.getValue().getLinks()!==null && this.resource.getValue().getLinks().length>0){
      return this.resource.getValue().getLinks().map(
        interaction => (
          {
            rel: interaction.rel,
  ...
  private handleDelete(body: string) {
    ...
    return this.httpClient.delete(this.url, { headers: this.headers }).subscribe(resp => {
      this.resource.next({
        ...HalResource(resp)
      });
      this.errorMessage.next(null);
