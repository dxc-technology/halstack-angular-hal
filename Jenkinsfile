        ...
        stage('Build and Deploy') {
           ...
           stages {
                ...
                steps {
                        script {
                            env.OLD_RELEASE_NUMBER = sh (
                                script: "grep 'version' projects/diaas-angular-cdk-hal/package.json | grep -o '[0-9.].*[^\",]'",
                                returnStdout: true
                            ).trim()
                        }
                }
            ...
            stage('Build and Install lib dependencies'){
                steps {
                    sh '''
                        cd ./projects/diaas-angular-cdk-hal && rm -rf node_modules
                        npm install
            ...
            stage('Publish dxc-ngx-hal alpha version to Artifactory ') {
                ...
                steps {
                    ...
                    script{
                      sh "sed -i -e 's/${OLD_RELEASE_NUMBER}/'${OLD_RELEASE_NUMBER}-alpha.${BUILD_ID}'/g' ./dist/diaas-angular-cdk-hal/package.json"
            ...
            stage('Tagging version') {
                ...
                steps {
                    script {
                        ...
                        sh "sed -i -e 's/${OLD_RELEASE_NUMBER}/'${RELEASE_NUMBER}'/g' dist/diaas-angular-cdk-hal/package.json"
                        sh "sed -i -e 's/${OLD_RELEASE_NUMBER}/'${RELEASE_NUMBER}'/g' projects/diaas-angular-cdk-hal/package.json"
                        sh "git push --tags"
                    }
                }
            }
                ...
                steps {
                    script {
                        try {
                            ...
                            sh "cat CHANGELOG.md"
                            sh "git add CHANGELOG.md projects/diaas-angular-cdk-hal/package.json package.json"
